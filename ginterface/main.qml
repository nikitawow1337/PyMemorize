    ...
    ApplicationWindow {
        ...
        Text {
            ...
            font.italic: true
            font.pointSize: {
                (parent.width + parent.height) / 17 - (backend.len1 / 2)
        ...
        Rectangle {
            ...
            TextField {
                ...
                onTextChanged: {
                    ...
                    // font.pointSize change if next word
                    text.font.pointSize = (parent.width + parent.height) / 17 - (backend.len1 / 2)
                }
                Keys.onPressed:
                    //Qt::Key key = Qt::Key_F1
                    if (event.key == Qt.Key_F1)
                    //if (event.key == backend.key)
                        if (debug.visible) {
                            debug.visible = false
                            btn.text = "<font color='#ffffff'>" + "Hint"
                        }
